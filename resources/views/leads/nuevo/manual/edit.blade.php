                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="{{ $lead->cliente->nombre_completo }}"
                                        readonly>
                                </div>
                            </div>
                            <div class="col-md-4">
                                    ...
                                    <select name="marca_id" id="marca_id"
                                        ...
                                        @foreach ($marcasVehiculo as $marca)
                                            <option value="{{ $marca->id }}"
                                                {{ ($lead->modeloVehiculo->marca_id ?? old('marca_id')) == $marca->id ? 'selected' : '' }}>
                                                {{ $marca->nombre_marca }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="modelo_id" id="modelo_id"
                                        ...
                                        {{ !$lead->modelo_id && !old('marca_id') ? 'disabled' : '' }}>
                                        <option value="">Seleccione el modelo</option>
                                        @if ($lead->modelo_id || old('marca_id'))
                                            @foreach ($modelosVehiculo->where('marca_id', $lead->modeloVehiculo->marca_id ?? old('marca_id')) as $modelo)
                                                <option value="{{ $modelo->id }}"
                                                    {{ ($lead->modelo_id ?? old('modelo_id')) == $modelo->id ? 'selected' : '' }}>
        ...
        $(document).ready(function() {
            ...
            function configurarCamposRequeridos(tipo) {
                ...
                if (tipo === 'postventa') {
                    ...
                    // Asegurar que los campos Select2 sean focusables
                    setTimeout(function() {
                        $('#tipo_servicio_id').next('.select2-container').find('.select2-selection').attr(
                            'tabindex', '0');
                        $('#horario_cita').next('.select2-container').find('.select2-selection').attr(
                            'tabindex', '0');
                    }, 100);
                } else if (tipo === 'compra') {
            ...
            $('form').on('submit', function(e) {
                ...
                if (!isValid) {
                    ...
                    if ($('.alert-danger').length === 0) {
                        $('form').prepend('<div class="alert alert-danger"><ul><li>' + errores.join(
                            '</li><li>') + '</li></ul></div>');
                    }
    ...
    <script>
        $('#marca_id').change(function() {
            ...
            if (marcaId) {
                ...
                $.ajax({
                    url: "{{ route('leads.modelos.por.marca', ['marcaId' => ':marcaId']) }}".replace(
                        ':marcaId', marcaId),
                    type: 'GET',
                    success: function(data) {
                        ...
                        @if ($lead->modelo_id)
                            modeloSelect.val('{{ $lead->modelo_id }}');
                        @endif
                    }
                });
            } else {
        ...
        // Trigger change si ya hay una marca seleccionada
        @if ($lead->modeloVehiculo)
